...
void WriteImage_u8(clLabviewDevice *d, cl_mem ImageObject, size_t Width, size_t Height, size_t Depth,
	...
	*Error = clLabviewDevice::Error(clLabviewDevice::SanitizeDevice(d));
	...
	try{
		...
		unsigned char test = Data[Width*Height*Depth];
...
void ReadImage_u8(clLabviewDevice *d, cl_mem ImageObject, size_t Width, size_t Height, size_t Depth,
				size_t XOffset, size_t YOffset, size_t ZOffset, unsigned char *Data, double *Time, int *Error){
#ifndef NO_OPENCL
	*Error = clLabviewDevice::Error(clLabviewDevice::SanitizeDevice(d));
	if(*Error != 0)
